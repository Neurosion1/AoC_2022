...
namespace
{
  ...
  int step(const Factory& factory, State state)
  {
    ...
    // Stop building ore robots once we've started building geode robots.
    bool build_ore_robot = state.geode_robots_ == 0 && state.current_ore_ >= factory.ore_robot_ore_;
    ...
    // Prune branches where even building a new geode robot every minute won't improve our score.
    int remaining_minutes = total_minutes - state.minute_;
    int remaining_geodes  = remaining_minutes * (remaining_minutes + 1) / 2;
    if (remaining_minutes * state.geode_robots_ + remaining_geodes + state.current_geode_ < best_geode_) {
      return state.current_geode_;
    }
